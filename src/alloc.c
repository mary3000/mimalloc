...
#if (MI_DEBUG>0)
  if (!page->is_zero) { genmc_memset(block, MI_DEBUG_UNINIT, size); }
#elif (MI_SECURE!=0)
...
void _mi_block_zero_init(const mi_page_t* page, void* p, size_t size) {
  ...
  if (page->is_zero && size > sizeof(mi_block_t)) {
    ...
    // otherwise memset
    genmc_memset(p, 0, mi_usable_size(p));
  }
}
...
static mi_decl_noinline void _mi_free_block_mt(mi_page_t* page, mi_block_t* block)
{
  ...
  #if (MI_DEBUG!=0)
  genmc_memset(block, MI_DEBUG_FREED, mi_usable_size(block));
  #endif
...
static inline void _mi_free_block(mi_page_t* page, bool local, mi_block_t* block)
{
  ...
  if (mi_likely(local)) {
    ...
    mi_check_padding(page, block);
    #if (MI_DEBUG!=0)
    genmc_memset(block, MI_DEBUG_FREED, mi_page_block_size(page));
    #endif
...
void* _mi_heap_realloc_zero(mi_heap_t* heap, void* p, size_t newsize, bool zero) {
  ...
  if (mi_likely(newp != NULL)) {
    if (zero && newsize > size) {
      ...
      genmc_memset((uint8_t*)newp + start, 0, newsize - start);
    }
    genmc_memcpy(newp, p, (newsize > size ? size : newsize));
...
mi_decl_restrict char* mi_heap_strdup(mi_heap_t* heap, const char* s) mi_attr_noexcept {
  ...
  if (t != NULL) genmc_memcpy(t, s, n + 1);
  return t;
}
